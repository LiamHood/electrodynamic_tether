...
%         Em = 165e-3; % V/m
        delta = gamma1*Em(jj)*L(ii);
        Lstar = (((me*Em(jj))^(1/3))/(e*2^(7/3)))*(3*pi*sigma*ht/ninf)^(2/3);
        lt = L(ii)/Lstar;
    
        % non-dimensional parameters
        eps0 = Em(jj)/L(ii)*(12*LAMBDAt(ii)/(3*sin(2*phi(ii))^2-2*LAMBDAt(ii)))*(mum/mue_si)*(sigma/rhov);
...
figure
surf(Em, L*1e-3, epsilon, abs(epsilon), 'EdgeColor', 'none')
xlabel('E_m [V/m]')
...
figure
surf(Em, rad2deg(phi), epsilon, abs(epsilon), 'EdgeColor', 'none')
xlabel('E_m [V/m]')
ylabel('\Phi [degree]')
zlabel('\epsilon')
